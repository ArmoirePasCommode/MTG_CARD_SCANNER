...
import type { Card, NewCard } from '../types/api';
...
export type EntryStatus = (typeof STATUS)[keyof typeof STATUS];

let _idCounter = 0;
export const newId = (): string => `entry-${Date.now()}-${++_idCounter}`;
...
export interface ReviewEntry {
  ...
  isFoil: boolean;
  quantity: number;
  mergedFromBatch?: boolean;
  error?: string | null;
}

export const findExistingByScryfallId = (
  cards: Card[],
  scryfallId: string | null | undefined
): Card | null => (scryfallId ? (cards.find((c) => c.scryfallId === scryfallId) ?? null) : null);
...
interface ReviewStageProps {
  entries: ReviewEntry[];
  cards: Card[];
  onUpdate: (id: string, patch: Partial<ReviewEntry>) => void;
...
type ListItem = ReviewEntry & { _kind: 'pending' | 'recognized' | 'failed' };
...
export const ReviewStage = ({
  ...
  batchTags = [],
  onBatchTagsChange,
}: ReviewStageProps): React.JSX.Element => {
  ...
  const listData: ListItem[] = [
    ...sections.pending.map((e) => ({ ...e, _kind: 'pending' as const })),
    ...sections.recognized.map((e) => ({ ...e, _kind: 'recognized' as const })),
    ...sections.failed.map((e) => ({ ...e, _kind: 'failed' as const })),
  ];
      ...
      <FlatList<ListItem>
        ...
        renderItem={({ item }) => {
          if (item._kind === 'recognized') {
            ...
            const willMerge = existing != null && item.isFoil === (existing.isFoil === true);
            return (
              <RecognizedRow
                entry={item}
                onUpdate={onUpdate}
...
export const RecognizedRow = ({
  entry,
  onUpdate,
  onRemove,
  willMerge,
  existingQty,
}: RecognizedRowProps): React.JSX.Element => {
  ...
  return (
    <View style={rowStyles.container}>
      ...
      <Image source={{ uri: card.imageUrl ?? entry.uri ?? undefined }} style={rowStyles.img} />
      <View style={rowStyles.body}>
        ...
        {willMerge ? (
          <View style={rowStyles.mergeHint}>
            ...
            <Text style={rowStyles.mergeHintText}>
              ...
              {existingQty != null ? ` (${existingQty} → ${existingQty + entry.quantity})` : ''}
            </Text>
          </View>
        ) : null}
        <View style={rowStyles.controls}>
          ...
          />
          <FoilToggle value={entry.isFoil} onChange={(v) => onUpdate(entry.id, { isFoil: v })} />
...
export const PendingRow = ({ entry, onRemove }: PendingRowProps): React.JSX.Element => (
...
export const FailedRow = ({ entry, onRetry, onRemove }: FailedRowProps): React.JSX.Element => (
...
const styles = StyleSheet.create({
  flex: { flex: 1 },
  reviewListContent: { paddingHorizontal: 16, paddingBottom: 100 },
  reviewSummary: {
  ...
  reviewSummaryValue: { fontSize: 16, color: colors.text, fontWeight: '700' },
  batchTagsCard: {
    backgroundColor: colors.backgroundCardSolid,
    borderRadius: radius.lg,
    borderWidth: 1,
  ...
  batchTagsLabel: { fontSize: 13, fontWeight: '700', color: colors.text },
  batchTagsHint: { fontSize: 12, color: colors.textMuted, lineHeight: 16 },
  batchTagsRow: { marginTop: 4 },
  bottomBar: {
...
const rowStyles = StyleSheet.create({
  ...
  stripe: { width: 3 },
  img: { width: 64, height: 90, borderRadius: radius.sm, margin: 10, backgroundColor: '#000' },
  imgPlaceholder: {
    alignItems: 'center',
    justifyContent: 'center',
    backgroundColor: colors.backgroundCardSolid,
  },
  body: { flex: 1, paddingVertical: 10, paddingRight: 12, gap: 6 },
  ...
  price: { fontSize: 13, fontWeight: '700', color: colors.text, marginLeft: 'auto' },
  iconBtn: { padding: 4, borderRadius: radius.sm },
  iconBtnPressed: { backgroundColor: colors.primarySoft },
  pendingTitleWrap: { flexDirection: 'row', alignItems: 'center', gap: 8, flex: 1 },
  pendingText: { color: colors.textMuted, fontSize: 13, fontWeight: '500', flex: 1 },
  failedTitleWrap: { flexDirection: 'row', alignItems: 'center', gap: 6, flex: 1 },
  failedText: { color: colors.danger, fontSize: 13, fontWeight: '700', flex: 1 },
  failedDetail: { color: colors.text, fontSize: 12, lineHeight: 16 },
  retryBtn: {
  ...
  retryText: { color: colors.primaryAccent, fontSize: 12, fontWeight: '700' },
  mergedBadge: {
    flexDirection: 'row',
    alignSelf: 'flex-start',
    alignItems: 'center',
  ...
  mergedText: { fontSize: 10, fontWeight: '700', color: colors.primaryAccent },
  mergeHint: {
    flexDirection: 'row',
    alignSelf: 'flex-start',
    alignItems: 'center',
  ...
  mergeHintText: { fontSize: 10, fontWeight: '600', color: '#fbbf24' },
});
