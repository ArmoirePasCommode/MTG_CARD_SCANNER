...
const SetSectionHeader = ({ section }: SetSectionHeaderProps): React.JSX.Element => {
  const { setName, code, ownedUnique, totalCards } = section;
  const knownTotal = totalCards != null && totalCards > 0;
  const denominator = knownTotal ? totalCards : ownedUnique;
  const percent =
    knownTotal && totalCards != null
      ? Math.min(100, Math.round((ownedUnique / totalCards) * 100))
      : null;
      ...
      <View style={styles.header}>
        ...
        <View style={styles.statsWrap}>
          <Text style={styles.fraction}>
            {ownedUnique}
            <Text style={styles.fractionDenom}> / {knownTotal ? denominator : '—'}</Text>
          </Text>
          {percent !== null ? <Text style={styles.percent}>{percent}%</Text> : null}
        </View>
      </View>
      <View style={styles.barTrack}>
        <View style={[styles.barFill, { width: `${percent ?? 0}%` }]} />
      </View>
    </View>
  );
};
