...
import CollectionToolbar, { type CollectionFilters } from '../components/CollectionToolbar';
import SetSectionHeader, { type SetSection } from '../components/SetSectionHeader';
...
const sortCards = (list: Card[], sortKey: string): Card[] => {
  const arr = [...list];
  switch (sortKey) {
    case 'name-asc':
...
interface SetSectionData extends SetSection {
  data: Card[];
...
const StatBlock = ({ label, value, accent, isText = false }: StatBlockProps): React.JSX.Element => (
  <View style={statStyles.block}>
    <Text
      style={[statStyles.value, { color: accent }, isText && statStyles.valueText]}
      numberOfLines={1}
    >
      {value}
    </Text>
...
const HomeScreen = ({ navigation }: HomeScreenProps): React.JSX.Element => {
  ...
  const uniqueSetCodes = useMemo<string[]>(() => {
  ...
  const visible = useMemo<Card[]>(() => {
    ...
    if (filters.rarities.size) {
      list = list.filter((c) => filters.rarities.has(String(c.rarity ?? 'common').toLowerCase()));
    }
    if (filters.colors.size) {
      list = list.filter(
        (c) => Array.isArray(c.colors) && c.colors.some((col) => filters.colors.has(col))
      );
    }
    if (filters.foilOnly) {
  ...
  const setSections = useMemo<SetSectionData[]>(() => {
    if (viewMode !== 'set') return [];
    const groups = new Map<string, SetSectionData>();
  ...
  const renderCard = ({ item }: { item: Card }): React.JSX.Element => (
    <CardItem
      card={item}
      ...
      <SafeAreaView style={styles.flex} edges={['top']}>
        {viewMode === 'set' ? (
          <SectionList<Card, SetSectionData>
            ...
            renderSectionHeader={({ section }) => <SetSectionHeader section={section} />}
...
const statStyles = StyleSheet.create({
  block: { flex: 1, alignItems: 'center', paddingHorizontal: 4 },
...
const styles = StyleSheet.create({
  ...
  resultCount: { color: colors.textMuted, fontSize: 12, marginBottom: 4, marginLeft: 4 },
  ...
  fabModalBackdrop: { flex: 1, backgroundColor: 'rgba(0,0,0,0.55)', justifyContent: 'flex-end' },
  fabSheet: {
  ...
  fabMenuItemPressed: { backgroundColor: colors.primarySoft },
  fabMenuIconWrap: {
    width: 42,
    height: 42,
    borderRadius: 13,
  ...
  fabMenuLabel: { fontSize: 15, fontWeight: '600', color: colors.text },
  fabMenuSub: { fontSize: 12, color: colors.textMuted, marginTop: 1 },
});
