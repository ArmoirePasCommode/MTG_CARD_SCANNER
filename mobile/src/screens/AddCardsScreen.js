...
//   1x Sol Ring (Commander 2014) 236
...
const limitedConcurrency = async (tasks, limit = 4) => {
...
const AddCardsScreen = ({ navigation }) => {
  ...
  const switchTab = (key) => {
    ...
    if (entries.length > 0) {
      Alert.alert(
        'Switch tab?',
        'Switching tabs will clear your current queue.',
        [
      ...
      <SafeAreaView style={styles.flex} edges={['top']}>
        ...
        <View style={styles.topBar}>
          ...
          <Text style={styles.title}>
            {stage === 'review' ? 'Review & Save' : 'Add Cards'}
          </Text>
          {stage === 'input' ? (
            <Pressable
              onPress={() => entries.length > 0 && setStage('review')}
...
const SearchTab = ({
  ...
  const handlePick = useCallback(async (name) => {
    ...
    try {
      ...
      // Merge if same scryfallId already in entries
      setEntries((prev) => {
      ...
      setGlobalError(err?.message || 'Card not found');
    } finally {
      setLookingUp(false);
    }
      ...
      {entries.length > 0 ? (
        ...
        <View style={styles.searchEmptyState}>
          ...
          <Text style={styles.emptySubtitle}>
            Type a card name above. Each result is added to your queue — adjust
            quantity and foil before saving.
          </Text>
        </View>
      )}
    </View>
...
const PasteTab = ({
  ...
  const handleImport = useCallback(async () => {
    ...
    const parsed = lines.map(parseDecklistLine).filter(Boolean);
    if (parsed.length === 0) {
    ...
    // Create all pending entries up front so the user sees feedback immediately
    ...
    // Resolve in parallel with a concurrency cap
    const tasks = newEntries.map((entry) => async () => {
      try {
        ...
        if (!card) {
          card = await fetchCardByName(p.name, true);
        }
        if (!card) throw new Error(`"${p.name}" not found`);
        // Merge within batch by scryfallId
        setEntries((prev) => {
          ...
          if (dup) {
            return prev
              .map((e) =>
                e.id === dup.id
                  ? { ...e, quantity: (e.quantity || 1) + (entry.quantity || 1), mergedFromBatch: true }
                  : e
              )
        ...
        setEntries((prev) =>
          prev.map((e) =>
            e.id === entry.id
              ? { ...e, status: STATUS.FAILED, error: err?.message || 'Not found' }
              : e
          )
        );
      }
  ...
  const showInput = entries.length === 0 || text.length > 0;
